	options(scipen=999)
	project_directory <- "/lustre/scratch/jmanthey/03_ethiopia_popgen/03_vcf"
	directory_name <- "eth_tree_100kbp"
	cluster <- "quanah"
	max_number_jobs <- 200
	
	# read in reference index
	# filtered to only include genotyped chromosomes
	ref_index <- read.table("GCF_000151805.1_Taeniopygia_guttata-3.2.4_genomic.fna.fai", stringsAsFactors=F)
	ref_index  <- ref_index[ref_index[,2] >= 1000000, ]
	...
	header <- paste('\tgunzip -cd ', project_directory, '/NC_011493.1.g.vcf.gz | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")

...
# make sure reference is indexed with bwa and samtools before use, and use CreateSequenceDictionary in GATK 
	...
	for(a in 1:nrow(individuals)) {
		if(a == 1) {
			helper1 <- cbind(faidx[,1], as.character(faidx[,3]), rep(a, nrow(faidx)))
		} else {
			helper1 <- rbind(helper1, cbind(faidx[,1], as.character(faidx[,3]), rep(a, nrow(faidx))))
		}
	}
	...
	for(b in 1:nrow(individuals)) {
		if(b == 1) {
			vcf_total <- paste("-V ", project_directory, "/02_vcf/", "${name_array}", "._", b, "_.g.vcf", sep="")
		} else {
			vcf_total <- paste(vcf_total, " -V ", project_directory, "/02_vcf/", "${name_array}", "._", b, "_.g.vcf", sep="")
		}
	}

...
# make sure reference is indexed with bwa and samtools before use, and use CreateSequenceDictionary in GATK 
	...
	for(a in 1:nrow(faidx)) {
		...
		if(length(job_number) > 1) {
			...
			for(b in 1:length(job_number)) {
				helper <- rbind(helper, c(paste(faidx[a,1], ":", interval_start, "-", interval_end, sep="")))
			...
			helper <- rbind(helper, paste(faidx[a,1], ":", 1, "-", faidx[a,2], sep=""))
		}
	}
	for(a in 1:nrow(individuals)) {
		if(a == 1) { 
			helper1 <- cbind(helper, rep(a, nrow(helper)))
		} else {
			helper1 <- rbind(helper1, cbind(helper, rep(a, nrow(helper))))
		}
	}
	...
	write("#$ -pe sm 2", file=a.script, append=T)
	...
	gatk_command <- paste('/lustre/work/jmanthey/gatk-4.1.0.0/gatk --java-options "-Xmx16g" HaplotypeCaller -R ', reference_genome_location, " -I ", a_name, " -ERC GVCF -O ", project_directory, "/02_vcf/", "${chr_array}", "._${ind_array}_.g.vcf", " --QUIET --intervals ", "${chr_array}", sep="")
	...
	write(unique(helper1[,1]), file=paste(directory_name, "/02b_gatk_database", "/helper3.txt", sep=""), ncolumns=1)

	
	
	# step 2
	# create genotyping database for each of the chromosomes segments
	...
	write(paste("#$ -t 1:", length(unique(helper1[,1])), sep=""), file=a.script, append=T)
	write("", file=a.script, append=T)
	write("module load intel java", file=a.script, append=T)
	write("", file=a.script, append=T)
	...
	#gatk 4.0
	gatk_command <- paste('/lustre/work/jmanthey/gatk-4.1.0.0/gatk --java-options "-Xmx20g" GenomicsDBImport ', vcf_total, " --genomicsdb-workspace-path ", project_directory, "/02_vcf/", "${chr_array}", " -L ", "${chr_array}", sep="")
	...
	write(paste("#$ -q ", queue, sep=""), file=a.script, append=T)
	write("#$ -pe sm 14", file=a.script, append=T)
	write(paste("#$ -P ", cluster, sep=""), file=a.script, append=T)
	write("#$ -l h_rt=48:00:00", file=a.script, append=T)
	write("#$ -l h_vmem=8G", file=a.script, append=T)
	...
	#gatk 4.0
	gatk_command <- paste('/lustre/work/jmanthey/gatk-4.1.0.0/gatk --java-options "-Xmx112g" GenotypeGVCFs -R ', reference_genome_location, " -V gendb://", project_directory, "/02_vcf/", "${interval_array}", " --include-non-variant-sites -O ", project_directory, "/03_vcf/", "${name_array}", ".g.vcf", " -L ", "${interval_array}", sep="")
	write(gatk_command, file=a.script, append=T)
